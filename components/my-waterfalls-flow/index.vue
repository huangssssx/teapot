<template>
  <view class="waterfalls-flow">
    ...
    >
      <view
        :class="['column-value', 'mod', { 'column-value-show': item2.o }]"
        v-for="(item2, index2) in columnValue(index)"
        :key="index2"
        :style="[s1]"
        @click.stop="wapperClick(item2)"
      >
        <view class="inner" v-if="data.seat == 1"> </view>
        <div class="body">
          <image
            mode="widthFix"
            :class="[
        ...
        <div class="main">
          <div class="tag">{{ item2.title }}「{{ item2.author }}」</div>
          <div class="view">
            <text class="label">{{ item2.desc }}</text>
          </div>
        </div>
        <div class="footer word" @click="inquiryClick(item2)">立即询价</div>

        <view class="inner" v-if="data.seat == 2"> </view>
      </view>
    </view>
  </view>
</template>
...
.mod {
  ...
  .main {
    ...
    .view {
      ...
      background-color: rgba(228, 193, 92, 0.1);
      border-radius: 20rpx;
